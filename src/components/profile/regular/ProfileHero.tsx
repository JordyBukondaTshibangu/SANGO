...
const ProfileHero:React.FC<ProfileHeroProps> = ({profile, firstname, lastname, position,   email,
  birthday, 
  address,
    phoneNumber }) => {
  ...
  return (
    <div className="bg-darkHeader w-full rounded-lg pb-5">
        <div className='relative bg-cover bg-no-repeat  flex flex-col  gap-5 md:gap-10 p-5 lg:p-10 justify-end min-h-[300px] rounded-tr-lg rounded-tl-lg' style={{ backgroundImage: `url(${profile})` }}></div>
            <div className="relative flex flex-col gap-5 px-5 lg:px-10 pb-5 ">
                <div className='self-center lg:self-start relative w-48 h-48 rounded-full p-1 bg-darkHeader -mt-24'>
                    <Image src={profile} width={200} height={200} alt="profile" className='h-full w-full rounded-full' />
                </div>
                <div className="flex flex-col lg:flex-row gap-2 justify-between">
                    <h4 className='text-2xl font-bold'>{firstname} {lastname}</h4>  
      ...
      <div className="flex lg:hidden items-center"  onClick={() => setViewMore(!viewMoreDetails)}>
        <span className='px-5 py-2'> { viewMoreDetails ? 'View Less' : 'View More'}</span>
        <RemoveRedEyeOutlinedIcon className='text-lg' />
</div>
      {
        viewMoreDetails &&           <div className="flex flex-col gap-5 px-5 lg:px-10 py-5 lg:pb-5">
            <div className="flex items-center gap-2">
