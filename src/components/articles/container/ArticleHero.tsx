...
const ArticleHero: React.FC<ArticleHeroProps> = ({ article }) => {
  ...
  return (
    <div className="bg-cover bg-no-repeat min-h-[550px] rounded-xl flex flex-col  gap-5 md:gap-10 p-5 lg:p-10" style={{backgroundImage : `url(${image})`}}>
        <Chip label="Technolgy" onClick={handleClick} className='mt-auto bg-primary w-32 flex items-center justify-center py-2 px-3 rounded-md text-white text-[16px] hover:bg-black'/>
          <h1 className='text-2xl lg:text-4xl font-medium lg:leading-[46px] lg:w-3/5'>{title }</h1>
          <div className="flex flex-col md:flex-row md:items-center gap-5 lg:gap-8">
            <Avatar alt="Remy Sharp" src="" />
            <h5 className='text-lg lg:text-xl font-bold'>{author}</h5>
