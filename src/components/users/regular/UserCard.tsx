  ...
  return (
      <div className='w-full bg-darkHeader rounded-lg flex flex-col gap-5 items-center py-10 px-5'>
        <div className="flex justify-center items-center rounded-full w-20 h-20">
            <Image src={user.profile ? user.profile : Avatar} width={200} height={200} alt='' className='w-full h-full rounded-full object-cover' />
        </div>
        <div className="flex flex-col gap-4 items-center  py-4">
            <h4 className="text-lg font-medium text-center">{user.firstName} { user.lastName}</h4>
      ...
      <div onClick={() => setShowMore(!showMore)}>
        {showMore ? 
          <div className='flex justify-center gap-5 items-center cursor-pointer'>
            <p>Show less</p> 
            <KeyboardArrowUpIcon className='text-lg' /></div>
      ...
      </div>
      {
             showMore &&  <div>
                ...
                <CustomTabPanel value={value} index={0}>
                  <InfoPanel bio={user.bio} email={user.email} phoneNumber={user.phoneNumber}  address={user.address}/>
                </CustomTabPanel>
