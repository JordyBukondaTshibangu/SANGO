...
const ArticleCard: React.FC<EventCardProps> = ({ event }) => {    
  ...
  return (
    <Link href={`events/${event.id}`} className='flex-1 flex min-w-[220px] lg:min-w-[248px] max-w-[348px] min-h-[394px] max-h-[394px] bg-cover rounded-lg shadow-xl hover:shadow-2xl hover:scale-105 transition duration-300 ease-in-out cursor-pointer  flex-col justify-end items-center gap-3 pb-10 px-10 text-center' style={{ backgroundImage: `url(${event.image})` }}>
