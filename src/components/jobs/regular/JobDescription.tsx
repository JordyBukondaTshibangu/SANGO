...
const JobDescription:React.FC<JobDescriptionProps> = ({description, requirements, desirables, benefits}) => {
  return (
    <div className='w-full flex flex-col gap-20 mt-5'>
        ...
        <div className="flex flex-col gap-6">
              ...
              <ul className='flex flex-col gap-2 list-disc px-4'>
                  {
                      requirements.map((requirement: string, index: number) => <li key={index}>{ requirement }</li>)
                  }
              </ul>
        </div>
        <div className="flex flex-col gap-6">
              ...
              <ul className='flex flex-col gap-2 list-disc px-4'>
                  {
                      desirables.map((desirable: string, index: number) => <li key={index}>{ desirable }</li>)
                  }
              </ul>
        </div>
        <div className="flex flex-col gap-6">
