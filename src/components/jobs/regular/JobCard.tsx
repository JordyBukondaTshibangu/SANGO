...
const JobCard:React.FC<JobCardProps> = ({job}) => {
  return (
    <Link href={`/jobs/${job.id}`} className='flex flex-col gap-5 bg-dark border-solid border-2 border-darkHeader shadow-lg p-5 lg:p-10  lg:h-[280px] cursor-pointer rounded-lg'>
      <div className="flex justify-between">
        <h3 className='text-lg'>{job.title}</h3>
        <BookmarkBorderOutlinedIcon className='self-end'/>
      </div>
      <div className='flex flex-col gap-2'>
        <p className='uppercase font-medium text-sucess text-xs lg:text-sm '>{job.period}</p>
